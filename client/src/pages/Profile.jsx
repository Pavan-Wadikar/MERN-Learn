  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      ...
      <input onChange={(e)=>setFile(e.target.files[0])} type="file" hidden accept="image/*" ref={fileRef}/>
        ...
        <p className="text-sm self-center">{fileUploadErr?
        <span className="text-red-700">Error uploading file</span>:filePercentage>0&&filePercentage<100?
        <span className="text-slate-600">
          {`Uploading ${filePercentage}%`}
        </span>:filePercentage==100?
        <span className="text-green-600">Image Uploaded Successfully</span>:""}</p>
        ...
        <button className="bg-slate-600 text-white p-4 rounded-xl uppercase hover:opacity-95 disabled:opacity-80">Update</button>
      </form>
